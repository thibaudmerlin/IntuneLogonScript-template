...
function Set-DriveMapping {
    ...
    if (($netConn.PingSucceeded) -and ($netConn.NameResolutionSucceeded)) {
        if($uncPath -like "*%*")
        {
            $uncPath = $uncPath.split("%")[0] + $env:USERNAME
        }
            ...
            if (Get-PSDrive | where-object { $_.Name -eq $driveLetter }) {
                if (Get-PSDrive | where-object { ($_.Name -eq $driveLetter) -and ($_.DisplayRoot -eq $uncPath) }) {
                    ...
                    if ($WhatIfPreference) {
                        ...
                        $netuseProcess = Execute-Process -FileName "net" -Arguments "use $driveLetter`: `"$uncPath`" /PERSISTENT:YES" -PassThru -Wait
                            if ($netuseProcess.ExitCode -ne 0){
                                throw "ExitCode = [$($netuseProcess.ExitCode)]. $($netuseProcess.StandardError.ReadToEnd())"
                            }
                            else{
                                Write-Host "Network drive [$driveLetter] - [$uncPath] successfully added."
                            }
...
try {
    ...
    # Cleaning Map Drives that are created through New-PSDrive command, there are not persistent -  FTA 15.05.2023
    Write-Host "Cleaning existing drive mappings.."
    $AllMapDrives = Get-CimInstance -ClassName Win32_NetworkConnection
    Foreach($MapDrive In $AllMapDrives)
    {
        If($MapDrive.ResourceType -eq "Any")
        {
            Write-Host "Removing $($MapDrive.LocalName).."
            Start-Process -FilePath "net" -ArgumentList "use /delete $($MapDrive.LocalName) /yes" -Wait
        }
    }
...
try {
    ...
    if ((Get-ScheduledTask -TaskName "$client`_Logonscript" -TaskPath "\" -ErrorAction SilentlyContinue)) {
        ...
        $TaskPrincipal = New-ScheduledTaskPrincipal -GroupId S-1-5-4
